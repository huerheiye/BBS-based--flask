...
#
#     u = User.one(username=username)
#     log('username', username, u)
#     # us = User.all(username=username)
#     topics = Topic.all(user_id=u.id)
#     log('topics', topics)
#     topics = sorted(topics, key=lambda m: m.created_time, reverse=True)
#     log('topics2', topics)
#     my_replys = Reply.all(user_id=u.id)
#     log('myre', my_replys)
#     my_topics = []
#     for my_reply in my_replys:
#         my_topic = Topic.one(id=my_reply.topic_id)
#         my_topics.append(my_topic)
#         log('timeis ', my_topic, my_reply.topic_id)
#         # my_topic.created_time,
#     # log('mytopics', my_topics)
#     # if my_topics is None:
#     #     return
#     my_topics = sorted(my_topics, key=lambda m: m.created_time, reverse=True)
#
#     if u is None:
#         abort(404)
#     else:
#         return render_template('profile.html', u=u, topics=topics, my_topics=my_topics)
#     # u = User.one(id=id)
#     # if u is None:
#     #     abort(404)
#     # else:
...
def update_sgin():
    ...
    u.update(u.id, signature=new_signature, location=new_location, blog=new_blog, github=new_github)
    return redirect(url_for('.setting'))
...
@main.route('/image/uodate', methods=['POST'])
def avatar_update():
